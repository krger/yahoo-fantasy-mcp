...
- **Runtime host:** Librarian (Debian VM, part of the Hyper-V stack on MOOSE)
- **Public endpoint:** `https://fantasy.krger.me/mcp`
...
- **Owner team:** team **#5**, "Lincolnshire Poachers."
- **Team-number map** (1–10): 1=Colesville Coves, 2=Bangers, 3=BlackTiger Snake, 4=Braxton's Bats, 5=Lincolnshire Poachers, 6=Hazel, 7=Perfect Pitch MICHAEL, 8=RearFang Lyre Snake, 9=Tim's Zoomers, 10=Squirrel.
...
- `yahoo_get_standings` — league standings. Returns `standings` as a normalized list (via `_parse_standings`): each team has numeric `rank`, `playoff_seed`, a structured `record` (`wins`/`losses`/`ties`/`pct`), and `games_back` (`null` for the leader). The standings feed has no per-category stats, so none are included (unlike the matchup/scoreboard breakdowns).
