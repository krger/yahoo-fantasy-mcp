...
- **Points-league (football) parsing is now *partly* live-verified (2026-09-07).** A drafted NFL points league (`1529868`, `league_key` `470.l.1529868`) exists on the account, and the tools were exercised against it: `is_points_league` detection from a real `stat_modifiers` block, the `scoring: "points"` field on matchup + scoreboard, `points`/`projected_points` (real values, so `team_projected_points` reads correctly), standings surfacing `points_for`/`points_against` with no category ranking, NFL label derivation in free-agent `stats` (`Pass Yds`, `Rec TD`, …), and `sort=PTS` returning results (the positive half of the categories-league caveat). **Still unverified: winner resolution and `stat_lines`.** Week 1 was `preevent` when checked, so `winner_team_key`/`is_tied` → `result`/`winner` were all `null` and `stat_lines` was `[]` — the two pieces the `*_POINTS` fixtures assert most specifically. Re-check after a week is actually played (NFL week 1 ends 2026-09-14) and correct the fixtures/parsers if the populated shape differs.
