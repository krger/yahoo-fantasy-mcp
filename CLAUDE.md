...
- **Yahoo gated Fantasy Sports API access behind an approval process (~2026-07-24) — every endpoint returns `403 "This application is not authorized to perform this action"` until the app is approved.** `developer.yahoo.com/fantasysports/guide/` now 308-redirects to <https://sports.yahoo.com/developer/>: self-serve app creation no longer grants Fantasy Sports permission, and apps created under the old flow lost access at the cutover. **This is not a credentials problem and no local change fixes it.** Diagnostic: if the `refresh_token` grant against `api.login.yahoo.com/oauth2/get_token` returns **200** but `/fantasy/v2/game/{code}` returns **403**, it is authorization at Yahoo, never local creds. Ruled out on 2026-07-24 (don't repeat): re-saving the app with "Fantasy Sports - Read" still checked, minting a fresh access token, and a complete fresh authorization-code grant — all still 403. **Do not delete or re-create the Yahoo app**: the create form no longer offers the Fantasy Sports permission, so a new app is strictly worse, and deleting destroys the **App ID** the access request needs. The fix is the request at <https://sports.yahoo.com/developer/access/> (read-only is the default; supply the existing App ID to get *that* app approved, preserving the Client ID/Secret and user grant). Once approved the existing credentials resume working with **no redeploy or restart**.
