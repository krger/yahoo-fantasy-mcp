...
- **Yahoo's Fantasy API `403` gating (~2026-07-24) is RESOLVED — access was restored by 2026-07-25 with no change on our side.** For ~a day every endpoint returned `403 "This application is not authorized to perform this action"` after Yahoo moved the API behind an approval process (`developer.yahoo.com/fantasysports/guide/` now 308-redirects to <https://sports.yahoo.com/developer/>, and self-serve app creation no longer grants Fantasy Sports permission). Verified healthy again on 2026-07-25: all 14 tools returned live data through the connector. **Keep the diagnostic for any future blanket `403`:** if the `refresh_token` grant against `api.login.yahoo.com/oauth2/get_token` returns **200** but `/fantasy/v2/game/{code}` returns **403**, it is authorization at Yahoo, never local creds — no code, credential, or deploy change fixes it, and recovery needs no redeploy or restart. **Do not delete or re-create the Yahoo app** (the create form no longer offers the Fantasy Sports permission, so a new app is strictly worse, and deleting destroys the **App ID** an access request needs); if it recurs, request approval for the existing app at <https://sports.yahoo.com/developer/access/>. Don't repeat the 2026-07-24 dead ends: re-saving the app with "Fantasy Sports - Read" checked, minting a fresh access token, and a full fresh authorization-code grant all still 403'd.
