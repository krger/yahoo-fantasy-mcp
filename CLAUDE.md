...
- `yahoo_get_standings` — league standings. Returns `standings` as a normalized list (via `_parse_standings`): each team has numeric `rank`, `playoff_seed`, a structured `record` (`wins`/`losses`/`ties`/`pct`), and `games_back` (`null` for the leader, and for **every** team in a points league — Yahoo omits the field there entirely, confirmed against the raw standings 2026-09-15; don't compute one). In a **categories** league each team also gets a `categories` list of season totals with the team's league `rank` per category (rate stats like ERA/WHIP ranked low-first), from a separate `league/{key}/teams/stats` call that degrades gracefully (standings still return without `categories` if it fails). In a **points** league that call is skipped (category ranking is meaningless); each team instead carries `points_for`/`points_against` (from Yahoo's `team_standings`) plus a `streak` (`{type, length}`, re-emitted from Yahoo's `{type, value}`). `streak` is surfaced **only when Yahoo sends it** — a categories league gets no `streak` key at all (not a null one), the same rule as `points_for`; verified against the raw standings of both leagues 2026-09-15.
...
- `yahoo_search_free_agents` — available free agents. Each player includes a `stats` map of season totals per scoring category (labeled via the league-derived `ScoringConfig`, numbers coerced; ratio stats like baseball's H/AB kept as strings), fetched inline via `;out=percent_owned,stats` and parsed in `_flatten_raw_yahoo_player`. The `sort` key resolves via `_resolve_sort`: named sorts (AR/OR/…), a numeric stat id, the baseball `_STAT_NAME_TO_ID` table, then a sport-neutral fallback that matches the league's own category labels (so football labels like `"Pass Yds"` sort with no per-sport table). `time_period` (`season`/`lastweek`/`lastmonth`/`biweekly`) sets the Yahoo `sort_type` for stat-based sorts — used to surface recent-form pickups; only `season` sends `sort_season`. That season comes from `_season_for` — the **resolved league's own** season from discovery, falling back to a pinned `YAHOO_SEASON` and then the local calendar year. Per-league on purpose: a season belongs to the league, not the wall clock (an NFL season runs into January, so the calendar year is wrong for a football league for weeks), and one deployment serving `mlb,nfl` can need two different seasons at once — which a single configured value cannot express. The displayed `stats` remain season totals; the window only controls ranking order.
...
- **Points-league (football) parsing is live-verified (2026-09-15), with one standing caveat: `stat_lines` is always empty.** The NFL points league on the account (`1529868`, `league_key` `470.l.1529868`) was re-checked against a **settled** week 1 (`status: "postevent"`), closing the piece the 2026-09-07 pass could not reach: **winner resolution works** — `winner_team_key`/`is_tied` are populated at postevent, `yahoo_get_matchup` returned `result: "loss"` for team 4 (103.16 vs 132.1) and `yahoo_get_scoreboard` returned a real `winner` team_key on all five matchups, each the higher-scoring side. Standings also came back correctly coerced (numeric `rank`, `record.pct` `1.0`/`0.0`, real `points_for`/`points_against`) — the empty-string values seen pre-season were a *pre-season* artifact, not a `_parse_standings` coercion bug. Verified earlier and still good: `is_points_league` from a real `stat_modifiers` block, `scoring: "points"`, `points`/`projected_points`, NFL label derivation, `sort=PTS`.
- **`stat_lines` is `[]` in a points league and that is Yahoo's shape, not a parser bug — don't "fix" it by rewriting the parser.** Live NFL matchup/scoreboard team nodes carry `team_points` and `team_projected_points` but **no `team_stats` block**, so `_points_matchup_node` has nothing to build informational stat lines from (the same `team_node[1:]` scan reads `team_points` correctly, which is what rules out a parse failure). Yahoo aggregates points, not stat lines, per fantasy team in a points league. The `*_POINTS` fixtures' `team_stats` blocks were assembled on-spec and model a response shape **never observed live** — they are kept as the "if Yahoo ever serves it" case; `MATCHUP_NODE_POINTS_LIVE` in `tests/fixtures.py` pins the real (no-`team_stats`) shape, and `tests/test_parsers.py` asserts empty `stat_lines` alongside correct winner/points for it. Confirmed against the raw API on 2026-09-15, not just parsed output: the exact URLs yfa builds (`team/{key}/matchups;weeks=1`, `league/{key}/scoreboard;week=1`) carry only `win_probability`, `team_points` and `team_projected_points` per team, and even an explicit `team/{key}/stats;type=week;week=1` request returns just `team_points`/`team_projected_points` — so no `;out=` or alternate team-level call recovers stat lines. Note the tool docstrings and the `yahoo_get_matchup`/`yahoo_get_scoreboard` entries above still advertise `stat_lines` as populated for points leagues.
- **A team with a playoff **bye** has no matchup node, and `yahoo_get_matchup` reports it misleadingly.** `_parse_matchup` raises `ValueError("Matchup data not found in Yahoo response")`, which `_handle_error` pattern-matches on "not found" into `Error: Resource not found (ValueError). Check the ID or key.` — sending the caller to re-check a team number that was correct. Seen live 2026-09-07: league 60467 week 24 (`playoff_start_week` 24, 6 playoff teams) had only two matchups, and the #1 seed (team 5) was on a bye, so the no-`week` call failed while `yahoo_get_scoreboard` for the same week returned both matchups fine. Not a regression and not a week-resolution bug — `lg.current_week()` was correct. If touched, distinguish "no matchup this week (bye)" from "bad team key".
- **`yahoo_get_player_notes` never returns the `notes`/`count` its docstring promises.** The handler deliberately does *not* fetch a notes sub-resource — there's an inline comment that Yahoo 400s on `;out=notes` ("Invalid player resource notes requested") — and returns only `player`, `status`, `status_full`, `injury_note`. The docstring (which **is** the advertised MCP tool description) still describes `'notes' (list of {timestamp, note})` and `'count'`, so clients are told to expect fields that never arrive. Confirmed live 2026-09-07 against a player Yahoo flags with `has_player_notes: 1`. Fix the docstring, not the handler, unless Yahoo starts serving notes.
...
- **Security issues go through GitHub private vulnerability reporting**, never a public issue or PR — see `SECURITY.md` for the channel, scope, and the by-design items (no built-in auth, plaintext `oauth2.json`, the `MCP_ALLOWED_HOSTS` bare-`localhost` entry) that are not vulnerabilities.
